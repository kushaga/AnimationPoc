...
public class SpringIndicator extends FrameLayout {
    ...
    private LinearLayout tabContainer, allLayout;
    ...
    private void addParentView() {
        ...
        allLayout.setGravity(Gravity.TOP);
        addView(allLayout);
    ...
    private void addPointView() {
        ...
        LinearLayout layoutCircle = new LinearLayout(getContext());
        LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT, 0.7f);

        springView.setLayoutParams(layoutParams);
        float scale = getResources().getDisplayMetrics().density;
        int sizeInDp = 20;
        int dpAsPixels = (int) (sizeInDp * scale + 0.5f);
        springView.setPadding(0, dpAsPixels, 0, 0);

        allLayout.addView(springView);
        //addView(springView);
    }

    private void addTabContainerView() {
        tabContainer = new LinearLayout(getContext());
        tabContainer.setLayoutParams(new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT, 1.0f)); //layout for our text views only
        tabContainer.setOrientation(LinearLayout.HORIZONTAL);
        tabContainer.setGravity(Gravity.CENTER);
        allLayout.addView(tabContainer);
        //addView(tabContainer);
    }

    private void addTabItems() {
        LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
        layoutParams.weight = 1/viewPager.getAdapter().getCount();

        tabs = new ArrayList<>();
        Log.e(this.getClass().toString(), viewPager.getAdapter().getCount() + "");
